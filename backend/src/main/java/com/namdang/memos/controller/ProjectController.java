...
public class ProjectController {
    ...
    public ApiResponse<String> createProject(
        ...
        return ApiResponse.<String>builder().result("Create Project Successfully").build();
    }

    @GetMapping("/project/{id}")
    @PreAuthorize(value = "@projectPermission.canViewAndUpdateProject(#id, authentication)")
    public ApiResponse<CreateProjectResponse> getProject(@PathVariable UUID id) {
        CreateProjectResponse response = projectService.getProject(id);
        return ApiResponse.<CreateProjectResponse>builder().result(response).build();
    }

    @GetMapping("/projects")
    public ApiResponse<List<CreateProjectResponse>> getProjects(@AuthenticationPrincipal Jwt jwt) {
        String email = jwt.getSubject();
        List<CreateProjectResponse> response = projectService.getProjects(email);
        return ApiResponse.<List<CreateProjectResponse>>builder().result(response).build();
    }

    @PutMapping("/project/{id}")
    @PreAuthorize(value = "@projectPermission.canViewAndUpdateProject(#id, authentication)")
    public ApiResponse<CreateProjectResponse> updateProject(
            @PathVariable UUID id, @RequestBody CreateProjectRequest request) {
    ...
    public ApiResponse<String> deleteProject(@PathVariable UUID id) {
        ...
        return ApiResponse.<String>builder().result("Delete Project Successfully").build();
    }
}
